...
import br.com.megusta.pages.pedido.CadastroPedidoModel;
import br.com.megusta.pages.pedido.ImprimirPedido;

@SuppressWarnings("serial")
@Secure(id = "URL_EDIT_PEDIDO_ABERTO", permissionType = UrlPermission.class)
public class AlteraPedidoAberto extends Template {
	...
	public AlteraPedidoAberto(PageParameters sourcePageParameters) {
	...
	public AlteraPedidoAberto(PageParameters sourcePageParameters, Pedido pedido) {
		super(sourcePageParameters);
		this.pedido = pedido;
		this.init();
	}
	...
	public void init() {
		...
		this.models = this.populaModel(sorvetes, pedido);
		...
		final DropDownChoice<FormaPagamento> dropDown = ComponentHelper.createDropDown("formaPagamento", new PropertyModel<FormaPagamento>(this.pedido, "formaPagamento"), Arrays.asList(FormaPagamento.values()));
		...
		valorAReceber.add(new AjaxFormComponentUpdatingBehavior("onchange") {
			...
			protected void onUpdate(AjaxRequestTarget target) {
				Double valor = AlteraPedidoAberto.this.pedido.getValor();
				Double troco = AlteraPedidoAberto.this.pedido.getValorRecebido() - valor;
				AlteraPedidoAberto.this.pedido.setTroco(troco);
		...
		dropDown.add(new AjaxFormComponentUpdatingBehavior("onblur") {
			...
			@Override
			protected void onUpdate(AjaxRequestTarget target) {
				boolean isCredito = AlteraPedidoAberto.this.pedido.getFormaPagamento().equals(FormaPagamento.CREDITO);
				boolean isDebito = AlteraPedidoAberto.this.pedido.getFormaPagamento().equals(FormaPagamento.DEBITO);
				if (isCredito || isDebito) {
					Double valor = AlteraPedidoAberto.this.pedido.getValor();
					AlteraPedidoAberto.this.pedido.setValorRecebido(valor);
		...
		dropDown.add(new AjaxFormComponentUpdatingBehavior("onchange") {
			...
			@Override
			protected void onUpdate(AjaxRequestTarget target) {
				if (AlteraPedidoAberto.this.pedido.getFormaPagamento().isFree()) {
					AlteraPedidoAberto.this.pedido.setValorRecebido(0d);
		...
		final ListView<CadastroPedidoModel> modelView = new ListView<CadastroPedidoModel>("sorvetes", new PropertyModel<List<CadastroPedidoModel>>(this, "models")) {
			@Override
			protected void populateItem(ListItem<CadastroPedidoModel> item) {
				...
				InlinedTextField<Integer> inlinedTextField = new InlinedTextField<Integer>("listQuantidadeAComprar", new PropertyModel<Integer>(model, "quantidadeAComprar"));
	...
	private SubmitLink criarBotaoCalcularPedido() {
		return new SubmitLink("calcularPedido") {
			...
			public void onSubmit() {
				AlteraPedidoAberto.this.pedido.getSorvetes().clear();
				AlteraPedidoAberto.this.pedido.setValor(0d);
				for (CadastroPedidoModel model : AlteraPedidoAberto.this.models) {
					AlteraPedidoAberto.this.populaPedido(model);
				}
				AlteraPedidoAberto.this.somaCoberturaTopping();
				AlteraPedidoAberto.this.subtraiDesconto();
				AlteraPedidoAberto.this.resumoMaker();
	...
	private SubmitLink criarBotaoFecharPedido() {
		...
		return new SubmitLink("fecharPedido") {
			...
			public void onSubmit() {
				...
				AlteraPedidoAberto.this.somaCoberturaTopping();
				AlteraPedidoAberto.this.subtraiDesconto();
				if (AlteraPedidoAberto.this.pedido.getSorvetes().isEmpty()) {
					this.error("Para fechar um pedido, é preciso ter pelo menos um sorvete!");
				} else {
					AlteraPedidoAberto.this.pedido.setDataHora(new Timestamp());
					try {
						if (AlteraPedidoAberto.this.pedido.getFormaPagamento().isFree()) {
							AlteraPedidoAberto.this.pedido.setValorRecebido(0d);
							AlteraPedidoAberto.this.pedido.setValor(0d);
						}
						if (AlteraPedidoAberto.this.checkEmAberto) {
							AlteraPedidoAberto.this.pedido.setStatusPedido(StatusPedido.ABERTO);
						} else {
							AlteraPedidoAberto.this.pedido.setStatusPedido(StatusPedido.PAGO);
						}
						AlteraPedidoAberto.this.pedido.getPersister().save();
						AlteraPedidoAberto.this.setResponsePage(new AlteraPedidoAberto(new PageParameters()));
						if (AlteraPedidoAberto.this.checkImprime) {
							new ImprimirPedido().imprimir(AlteraPedidoAberto.this.pedido);
						}
	...
	private List<CadastroPedidoModel> populaModel(List<Sorvete> sorvetes, Pedido pedido) {
		...
		for (Sorvete sorvete : sorvetes) {
			...
			List<Sorvete> sorvetesPedido = pedido.getSorvetes();
			for (Sorvete sorvetePedido : sorvetesPedido) {
				if (sorvetePedido.getSabor().equalsIgnoreCase(sorvete.getSabor())) {
					model.setQuantidadeAComprar(model.getQuantidadeAComprar() + 1);
				}
			}
			models.add(model);
		}
